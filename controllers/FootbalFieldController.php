 ...
 * @author crist
 */
class FootbalFieldController {

    public static function getAllFootbalFieldRents() {
        $connect = BBDDController::getConnect();
        $sql = "SELECT * FROM footbal_field";
        $result = $connect->query($sql);
        while ($row = $result->fetch_object()) {
            $rents [] = new FootbalField($row->id_rent_field, $row->id_pista, $row->date, $row->entry_time, $row->closing_time, $row->id_usuario, $row->amount, $row->paid);
        }
        return $rents;
    }

    public static function getAllFootbalFieldRentsUser($user) {
        $connect = BBDDController::getConnect();
        $sql5 = "SELECT * FROM footbal_field WHERE id_usuario=" . $user . "";
    ...
    public static function rentPist($pista, $fecha, $e_time, $c_time, $user) {
        ...
        $sql = "SELECT id_pista FROM footbal_field WHERE  id_pista=" . $pista . " && date='" . $fecha . "' && entry_time='" . $e_time . "' && closing_time='" . $c_time . "' ";
        $result = $connect->query($sql);
        ...
        if (@$g == $pista) {
            ...
            if ($h < 3) {
                $sql = "INSERT INTO footbal_field ( id_pista, date, entry_time, closing_time, id_usuario, amount, paid ) VALUES ( " . $pista . ", '" . $fecha . "','" . $e_time . "','" . $c_time . "'," . $user . "," . $amount . " ,0);";
   ...
   $connect = BBDDController::getConnect();
                ...
                $sql = "DELETE FROM footbal_field WHERE id_pista=" . $pista . " && date='" . $fecha . "' && entry_time='" . $e_time . "' && closing_time='" . $c_time . "' && id_usuario=" . $user .";";
                $query = mysqli_query($connect, $sql);
                $sql = "UPDATE `users` SET `field_renting`=field_renting-1 WHERE id_user=" . $user . " ";
                $query = mysqli_query($connect, $sql);
                if ($query) {
